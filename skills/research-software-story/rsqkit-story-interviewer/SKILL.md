...
### YAML front matter
The front matter is exempt from the one-sentence-per-line rule: keep one key per line, and do not split a value across lines by sentence (`description` stays on its single line).
