...
__If Claude suggests new tool entries, copy the suggested YAML and add it to `_data/tool_and_resource_list.yml` into your RSQKit repository fork (or branch) before submitting your page as a pull request.__ (otherwise the site will not build)
...
A system prompt is available in `standalone/rsqkit-task-page-system-prompt.md`, copy and paste this into any LLM and then it should behave in the same way as the Claude skill, however sometimes you have to nudge it to do steps it might not have done but knows about.
